...
See `data/toronto_election_turnout/processed/qa_summary.json` for generated counts.
...
See `docs/granularity_audit.md` for the detailed check of atlas dots, polygon coverage, no-geometry rows, and no-turnout polygons.
...
This reference dataset is not a replacement for the polling-division turnout files in `data/toronto_election_turnout/processed/`. It is election-day-only for 2023 and excludes advance-poll votes, while this project retains advance, mail-in, and special reporting buckets where the official source supports them.

See `docs/zack_taylor_ct_comparison.md` for the full comparison and official-source reconciliation.
